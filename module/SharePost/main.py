...
class Module(ModuleBase):
	...
	def post_json(self, post):
		...
		#####
		post 		= yield self.site.db.post.find_one(query, {'user_id': 1, 'site_id': 1, 'time': 1, 'like': 1, 'post': 1, 'type': 1})
		#####
		if post:
	...
	def view_update(self, post):
		try:
			...
			yield self.site.db.post.update({
						"_id": post['_id'],
						"view.user": {"$nin": user_id}
					},{
						"$inc": {"view.count": 1},
						"$pushAll": { "view.user": user_id}
					},
					upsert= True
				)
